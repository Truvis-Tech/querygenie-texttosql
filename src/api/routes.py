...
async def generate_query(req: QueryRequest, user: dict = Depends(verify_token)):
    ...
    try:
        ...
        # Validation 3: Analytical intent validation
        ...
        if validation_res == 'False':
            logger.warning("VALIDATION_FAILED - Query not identified as analytical for user: %s", username)
            return JSONResponse(
                status_code=400,
        ...
        if validation_res == 'False':
            logger.warning("Invalid domain query detected")
            return JSONResponse(
                status_code=400,
                content={'result': [], 'metadata': "", 'sql_query': "", 'textual_summary': ["Sorry, it feels like query is out of my domain. Could you please try again?"], 'followup_prompts': [], "x-axis": "", "typeOFgraph": ""}
            ) 
                 
        # SQL Generation
        logger.info("Starting SQL generation using LLM for user: %s", username)
...
async def postgres_loader(request: MetadataRequest):
    ...
    try:
        db_util = GoogleCloudSqlUtility(request.market)
        # Use input_dir for data_output_directory, or add tables_path/columns_path to db_config.py if needed
        tables_path = db_util.input_dir  # Update if you have a specific tables_path property
        ...
        if request.metadata_type == "table":
            ...
            logger.info("POSTGRES_LOADER completed - %d table records inserted", record_count)
            ...
            logger.info("POSTGRES_LOADER completed - %d column records inserted", record_count)
        ...
        logger.exception("POSTGRES_LOADER_ERROR - Type: %s, Error: %s", request.metadata_type, str(e))
        raise HTTPException(status_code=500, detail=f"Unexpected error: {str(e)}")
...
@router.get("/test-cloudsql-connection")
def test_cloudsql_connection(payload: Payload):
    try:
        db_util = GoogleCloudSqlUtility(payload.market)
        result = db_util.select("SELECT 1;")
        if result:
            return JSONResponse(content={"status": "success", "result": result})
        else:
            return JSONResponse(content={"status": "fail", "message": "Connection failed or no result returned."}, status_code=500)
    except Exception as e:
